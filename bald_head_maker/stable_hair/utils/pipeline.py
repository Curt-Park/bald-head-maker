...
class StableHairPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def _encode_prompt(
        self, prompt, device, do_classifier_free_guidance, negative_prompt
    ):
    ...
    def images2latents(self, images, dtype):
        ...
        latents = (
            self.vae.encode(images)["latent_dist"].mean * self.vae.config.scaling_factor
        )
        return latents
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ref_image_latents = (
            torch.cat([ref_padding_latents, ref_image_latents])
            if do_classifier_free_guidance
